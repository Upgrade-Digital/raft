...
public class MessageTransport {
  ...
  public void receive() throws IOException {
    ...
    for (MessageHandler handler : handlers) {
      if (container.hasAppendEntries()) {
        handler.handleAppendEntries(container.getAppendEntries());
      }
      if (container.hasAppendResponse()) {
        handler.handleAppendResponse(container.getAppendResponse());
      }
      if (container.hasRequestVote()) {
        handler.handleRequestVote(container.getRequestVote());
      }
      if (container.hasVoteResponse()) {
        handler.handleVoteResponse(container.getVoteResponse());
      }
    }
  }
